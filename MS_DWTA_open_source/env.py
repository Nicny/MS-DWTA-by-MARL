...
class AirDefense(BaseEnvironment):
    def __init__(self, server_ip=None, server_port=None, agent_key_event_file=None, duration_interval=None,
        ...
        self.min_fire_range_1 = 3700
        self.fire_range_2 = 240900
        self.min_fire_range_2 = 7400
        self.fire_range_3 = 92600
        self.min_fire_range_3 = 3700
        self.fire_range_4 = 25500
        self.min_fire_range_4 = 370
    ...
    def get_weapon_num(self, agent_id):
        ...
        if health != 0:
            ship = self.blue_ship[agent_id]["unit"]
            total_weapon = ship.get_mounts()  # Return dic, key is the three mounted GUID, and value is the three CMount classes, not the number
    ...
    def weapon_permit(self, agent_id, own_dist, vessel_dist):
        ...
        if vessel_dist > self.border:    # >50km
            if agent_id in [3, 4, 5, 6]:    # beyond duty
                ...
                return weapon_permission
        elif own_dist > self.own_defense:    # 15km<dist<50km
            if agent_id in [0, 1, 2, 6]:    # beyond duty
                return weapon_permission
            else:    # In duty
                if self.min_fire_range_1 < own_dist < self.fire_range_1 and weapon_num_list[0] > 0:
                    weapon_permission[0] = 1
            ...
            if e_value == 1:
                ...
                for al_id in self.blue_ship:
                    ...
                    if al_health > 0:
                        ...
                        if weapon_permission != [0, 0, 0, 0]:  # The ship has the firing condition with the target
            ...
            if contact_type == 1:  # 1：missile
                ...
                if true_unit and unit.iWeaponsAimingAtMe == 0:
                    ...
                    if idx is not None:
                        ...
                        if self.red_missile[idx]["guid"] is None:    # prevent repeated calculation
    ...
    def compute_intercept_reward_list_1(self):
        ...
        file_newest_path = os.path.join(path, lists[-1])    # newest file
        wasted_file_path = os.path.join(path, lists[0])
        os.remove(wasted_file_path)    # Delete lua execution files
    ...
    def compute_intercept_reward_list_2(self):
        # For convenience, we complete the dynamic reward adjustment by processing the log
        path = "D:/Mozi/MoziServer/bin/Logs"
        lists = os.listdir(path)
        lists.sort(key=lambda fn: os.path.getmtime(path + "/" + fn))
