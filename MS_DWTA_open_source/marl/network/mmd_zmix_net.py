...
class MMD_ATT_MixNet(nn.Module):
    ...
    def forward(self, z_values, states):
        ...
        for i in range(self.n_attention_head):
            ...
            z_lambda = F.softmax(raw_lambda, dim=-1)
            z_lambda_list.append(z_lambda)

        # shape: [bs * max-len, n_attention_head, N]
        z_lambda_list = torch.stack(z_lambda_list, dim=1).squeeze(-2)
        ...
        z_values = z_values.permute(0, 1, 3, 2).view(-1, self.args.particle_num, self.args.n_agents)
        z_values *= z_lambda_list  # [bs * max-len, n_attention_head, N]
        ...
        hidden = F.elu(torch.bmm(z_values, w1) + b1)  # [bs * max_ep_len, 4, hidden_dim]
        z_total = torch.bmm(hidden, w2) + b2  # [bs * max_ep_len, 4, 1]
        z_total = z_total.permute(0, 2, 1)  # [bs * max_ep_len, 4, 1] --> [bs * max_ep_len, 1, 4]
        z_total = z_total.view(bs, -1, self.args.particle_num)  # [bs, max_ep_len, 4]
        return z_total
