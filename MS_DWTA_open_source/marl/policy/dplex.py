...
from mozi_ai_sdk.MS_DWTA_open_source.marl.network.dplex_net import DPLEXMixer
...
class DPLEX:
    def __init__(self, args):
        ...
        self.eval_qmix_net = DPLEXMixer(args)
        self.target_qmix_net = DPLEXMixer(args)
        self.args = args
        if self.args.cuda:
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/' + 'rnn_net_params.pkl'):
                ...
                path_qmix = self.model_dir + '/' + 'dplex_net_params.pkl'
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):
        ...
        x_mac_out[avail_u == 0] = -9999999
        max_action_qvals, max_action_index = x_mac_out.mean(dim=4).max(dim=3)
        ...
        cur_max_actions_onehot = torch.zeros(cur_max_actions.squeeze(3).shape + (self.args.n_actions,)).cuda()
        cur_max_actions_onehot = cur_max_actions_onehot.scatter_(3, cur_max_actions, 1)

        q_attend_regs = 0
        assert target_max_qvals.shape == (episode_num, max_episode_len, self.args.n_agents, self.n_target_quantiles)
        # Policy Q
        ans_chosen, q_attend_regs, head_entropies = self.eval_qmix_net(chosen_action_qvals, s, target=False, is_v=True)
        # Additional `actions_onehot`, `max_action_qvals`
        ans_adv, _, _ = self.eval_qmix_net(chosen_action_qvals, s, target=False, actions=u_onehot, max_q_i=max_action_qvals, is_v=False)
        # Q_tot in Eq.12 (of QPLEX paper)
        chosen_action_qvals = ans_chosen + ans_adv
        # Target Q
        target_chosen, _, _ = self.target_qmix_net(target_max_qvals, s_next, target=True, is_v=True)
        # Additional `actions_onehot`, `max_action_qvals`
        target_adv, _, _ = self.target_qmix_net(target_max_qvals, s_next, target=True, actions=cur_max_actions_onehot,
    ...
    def save_model(self, evaluate_num):
        ...
        torch.save(self.eval_qmix_net.state_dict(), self.model_dir + '/' + num + '_dplex_net_params.pkl')
