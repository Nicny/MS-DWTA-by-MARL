...
from mozi_ai_sdk.MS_DWTA_open_source.marl.network.ddn_net import DDNMixer
...
class DDN:
    def __init__(self, args):
        ...
        self.n_quantiles = 1
        self.n_target_quantiles = 1

        self.eval_rnn = BasicMAC(args)
        self.target_rnn = BasicMAC(args)
        self.eval_qmix_net = DDNMixer(args)
        self.target_qmix_net = DDNMixer(args)
        self.args = args
        if self.args.cuda:
    ...
    def save_model(self, evaluate_num):
        ...
        torch.save(self.eval_qmix_net.state_dict(), self.model_dir + '/' + num + '_ddn_net_params.pkl')
