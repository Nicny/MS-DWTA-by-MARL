...
class Agents:
    def __init__(self, args):
        ...
        if args.alg == 'qtran_base':
            ...
            from policy.addr import ADDR
            self.policy = ADDR(args)
        elif args.alg == 'dmix':
            from policy.dmix import DMIX
            self.policy = DMIX(args)
        elif args.alg == 'dplex':
            from policy.dplex import DPLEX
            self.policy = DPLEX(args)
        elif args.alg == 'ddn':
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None):
        ...
        elif self.args.alg == "dmix" or self.args.alg == "dplex" or self.args.alg == "ddn":
            action = self.policy.eval_rnn.choose_action_(inputs, avail_actions, epsilon, agent_num)
            return action
        else:
            hidden_state = self.policy.eval_hidden[:, agent_num, :].cuda()
