...
class IQNRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.n_target_quantiles = args.n_quantiles
        if self.args.alg == 'ddn':
            self.n_quantiles = args.ddn_n_quantiles
            self.n_target_quantiles = args.ddn_n_quantiles
        self.n_approx_quantiles = args.n_approx_quantiles

        self.fc1 = nn.Linear(input_shape, args.rnn_hidden_dim)
        self.rnn = nn.GRUCell(args.rnn_hidden_dim, args.rnn_hidden_dim)
        self.phi = nn.Linear(args.phi_dim , args.rnn_hidden_dim)
    ...
    def forward(self, inputs, hidden_state, forward_type=None):
        ...
        if self.args.alg != "diql":
            ...
            q_phi = q_phi.unsqueeze(1).expand(-1, n_agents, -1, -1).contiguous().view(-1, self.args.rnn_hidden_dim)
